...
export default class QuadTree {
    ...
    insert(collider) {
        ...
        if(this._quadrants == null) {
            ...
            if(this._nodeCapacity < this._contents.length) {
                ...
                const cPoint = vec2.fromValues((nwPoint[0] + sePoint[0]) / 2,
                                               (nwPoint[1] + sePoint[1]) / 2);
                const cePoint = vec2.fromValues(sePoint[0],
                                                cPoint[1]);
                const csPoint = vec2.fromValues(cPoint[0],
                                                sePoint[1]);
                const cnPoint = vec2.fromValues(cPoint[0],
                                                nwPoint[1]);
                const cwPoint = vec2.fromValues(nwPoint[0],
                                                cPoint[1]);
    ...
    test(callback) {
        ...
        for(let i = 0; i < this._contents.length; i++) {
            ...
            if(this._parent)
                this._parent._parentTest(this._contents[i], callback);    
        }
    ...
    _parentTest(collider, callback) {
        ...
        if(this._parent)
            this._parent._parentTest(collider, callback);    
    }
}