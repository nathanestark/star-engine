...
export interface Device {
    name: string;
    triggerCall: (event: Event) => void;
    values: Map<string, boolean>;
}
export interface Bind {
    keys: Array<BindKey>;
    type: "action" | "value";
    command?: (on: boolean) => void;
    isFunction: boolean;
}
...
export interface TriggerCallEvent {
    key: string;
    value: boolean;
}
...
export default class InputController {
    ...
    unregisterDevice(
        deviceName: string,
        unregisterCall: (triggerCall: (event: Event) => void) => void
    ) {
        if (!this.devices.has(deviceName)) throw "Device '" + deviceName + "' is not registered.";
    ...
    bindCommand(
        ...
        if (keys instanceof Array)
            keys.sort((keyA, keyB) => {
                ...
                if (keyA.device < keyB.device) return -1;
                else if (keyA.device > keyB.device) return 1;
                else if (keyA.key < keyB.key) return -1;
                else if (keyA.key > keyB.key) return 1;
                return 0;
            });
        else keys = [keys];
    ...
    update() {
        ...
        for (let i = 0; i < off.length; i++) {
            ...
            if (curVal && curVal.length > 0) {
                for (let j = 0; j < curVal.length; j++) {
                    if (curVal[j].isFunction) curVal[j].command(false);
                }
            }
        }

        // Get a list of all pressed binds.
        const keys: Array<BindKey> = [];
        for (let [dKey, dValue] of this.devices) {
            for (let [vKey] of dValue.values) {
        ...
        const tried = new Map();
        let _testOnActions: null | ((keys: Array<BindKey>) => void) = null;
        _testOnActions = (keys) => {
            ...
            if (curVal && curVal.length > 0) {
                ...
                if (!this.on.has(key)) {
                    ...
                    for (let j = 0; j < curVal.length; j++) {
                        if (curVal[j].isFunction) curVal[j].command(true);
                    }
                }
            } else {
                // Then test for value.
