export default class Game {
    constructor(properties = {}) {
        ...
        if (properties.paused) this._paused = properties.paused;

        if (properties.timeScale) this._timeScale = properties.timeScale;

        if (properties.minUpdateTime) this._minUpdateTime = properties.minUpdateTime;
        ...
        for (let i = 0; i < tags.length; i++) {
            ...
            if (curResults) {
                ...
                for (let j = 0; j < curResults.length; j++) {
                    ...
                    if (
                        oldResults == null ||
                        Object.prototype.hasOwnProperty.call(oldResults, objId)
                    ) {
                        results[objId] = objId;
                    }
                }
    ...
    addTags(objId) {
        ...
        let obj = objId;
        if (typeof obj === "number") obj = this._gameObjects[obj];
        ...
        for (let i = 1; i < arguments.length; i++) {
            ...
            if (obj.tags) {
                ...
                if (obj.tags.indexOf(tag) == -1) obj.tags.push(tag);
            } else {
                obj.tags = [tag];
            }
    ...
    moveGameObject(id, newParent, callback) {
        if (id == 0 || id.id == 0) throw "Cannot move the root object.";
    ...
    addGameObject(obj, parent, callback) {
        ...
        if (typeof parent === "function") {
            ...
            if (parent.id) {
                ...
                if (this._gameObjects[parent.id]) parent = parent.id;
    ...
    removeGameObject(id, callback) {
        if (id == 0 || id.id == 0) throw "Cannot remove the root object from game.";
    ...
    _addGameObject(obj, parent, resolve, reject) {
        ...
        else if (typeof parent === "object") parentId = parent.id;
        ...
        while (toAdd.length > 0) {
            ...
            if (!cur.parent.children) cur.parent.children = [];
    ...
    _start() {
        ...
        gameLoop = function () {
            ...
            if (lastTime == null) lastTime = new Date().getTime();
