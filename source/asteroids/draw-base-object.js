...
export default class DrawBaseObject {
    ...
    _processImage(imgDef) {
        ...
        if (ret.image) {
            ...
            else ret.offsetSize = vec2.clone(imgDef.offsetSize);

            if (!(imgDef.clipPosition instanceof Float32Array)) ret.clipPosition = vec2.create();
            else ret.clipPosition = vec2.clone(imgDef.clipPosition);

            if (!(imgDef.clipSize instanceof Float32Array))
                ret.clipSize = vec2.fromValues(ret.image.width, ret.image.height);
            else ret.clipSize = vec2.clone(imgDef.clipSize);
        }
        if (imgDef.animations) {
            ...
            for (let name in imgDef.animations) {
                ...
                } else newAnimation.framesPerSecond = animation.framesPerSecond;
    ...
    _getFrame(animationTime) {
        let frame = 0;
        if (!this.image._startTime) this.image._startTime = animationTime;
        else
            frame = Math.floor(
                (animationTime - this.image._startTime) *
