...
import { default as InputController, CONTROLLER_ACTION, CONTROLLER_VALUE } from '../core/input-controller';
...
export default class Asteroids extends Game {
    constructor(canvas) {
        ...
        const collisionDetection = new CollisionDetection(this, [this.worldBounds.position, this.worldBounds.size]);
        ...
        this.controller.registerDevice("keyboard", (triggerCall) => {
        ...
        this.controller.registerDevice("mouse", (triggerCall) => {
    ...
    spawnAsteroids() {
        ...
        for(let x= 0; x < numAsteroids; x++) {
            ...
            const position = vec3.fromValues(1, this.worldBounds.size[0]/2 - radius,this.worldBounds.size[1]/2 - radius);
            ...
            this.addGameObject(
                new Asteroid(this, {
                    ...
                    velocity: vec2.fromValues(numAsteroids/3 * 50* (1-Math.random()*2), numAsteroids/3 * 50*(1-Math.random()*2)),
        ...
        destroy = (parent) => {
            ...
            if(parent.mass <= 5)
                return;
            else if(parent.mass == 50)
                newMass = 20;

            for(let x=0; x < 3; x++) {
                const vel = vec2.clone(parent.velocity);
                vec2.add(vel, vel, vec2.fromValues(50*(1-Math.random()*2), 50*(1-Math.random()*2)));
                this.addGameObject(new Asteroid(this, {
                    mass: newMass,
                    radius: newMass,
    ...
    spawnShip(on) {
        if(on && this.ship == null) {
            ...
            this.addGameObject(this.ship, this.worldObjects)
                .then(() => { this.camera.target = this.ship; });
            ...
            this.controller.bindCommand({device: 'keyboard', key: 65}, CONTROLLER_ACTION, rotateCounterClockwise);
            this.controller.bindCommand({device: 'keyboard', key: 68}, CONTROLLER_ACTION, rotateClockwise);
            ...
            destroy = () => {
                ...
                this.controller.unbindCommand({device: 'keyboard', key: 87}, CONTROLLER_ACTION, thrust);
                this.controller.unbindCommand({device: 'keyboard', key: 65}, CONTROLLER_ACTION, rotateCounterClockwise);
                this.controller.unbindCommand({device: 'keyboard', key: 68}, CONTROLLER_ACTION, rotateClockwise);
                ...
                if(this.cLives > 0){
                        ...
                        this.addGameObject(fireMsg, this.centeredHud).then(() => {
                            const fireToSpawn = (on) => {
                                if(on) {
                                    ...
                                    this.controller.unbindCommand({device: 'mouse', key: 1}, CONTROLLER_ACTION, fireToSpawn);
                    ...
                    this.addGameObject(gameOver, this.centeredHud).then((obj) => {
                        ...
                        restart = () => {
                            ...
                            this.controller.unbindCommand({device: 'mouse', key: 1}, CONTROLLER_ACTION, restart);

                            this.startGame();
                        };
                        setTimeout(() => {
                            this.controller.bindCommand({device: 'mouse', key: 1}, CONTROLLER_ACTION, restart);
                        }, 1000);
                    });
                }
            };
