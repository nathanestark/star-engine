...
class Math2D {
    static pointInPolygon(point, polygonPoints) {
        ...
        if(Math2D.pointInBoundingBox(point, Math2D.createBoundingBoxFromPoly(polygonPoints))) {
            ...
            for(let p = 0; p < polygonPoints.length; p++) {
                ...
                if(lSeg2[0] != null && Math2D.intersectingLines(lSeg1, lSeg2))
                    inside = !inside;
            }
            ...
            if(Math2D.intersectingLines(lSeg1, lSeg2))
                inside = !inside;
    ...
    static boundingBoxIntersectsBoundingBox(boxPointsA, boxPointsB) {
        ...
        return Math.abs(boxPointsA[0][0] - boxPointsB[0][0]) * 2 < (aWidth + bWidth) 
    ...
    static circleOnBoundingBox(point, radius, boxPoints) {
        if(!Math2D.circleIntersectsBoundingBox(point, radius, boxPoints))
            ...
            if((point[0] - boxPoints[0][0]) > radius
    ...
    static boundingBoxOnBoundingBox(boxPointsA, boxPointsB) {
        ...
        if(Math2D.pointInBoundingBox(boxPointsA[0], boxPointsB)
    ...
    static intersectingLines(lineAPoints, lineBPoints) {
        ...
        return (turn(lineAPoints[0], lineBPoints[0], lineBPoints[1]) 
    ...
    static inflateBoundingBox(boxPoints, width, height) {
        if(typeof(height) === 'undefined')
            height = width;
        return [vec2.fromValues(boxPoints[0][0]-width,
                                boxPoints[0][1]-height), 
                vec2.fromValues(boxPoints[1][0]+width,
                                boxPoints[1][1]+height)];
    }
    ...
    static calculateElasticCollisionVelocity(out,
                                             curVelocity,
                                             normal,
    ...
    static calculateInelasticCollisionVelocity(out,
                                             curVelocity,
                                             normal, 
                                             obj1Velocity, 
                                             obj1Mass = 1,
